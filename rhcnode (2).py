...
class RHCNode(rhcbase.RHCBase):
    def __init__(self, dtype, params, logger, name):
        ...
        self.do_profile = self.params.get_bool("profile", default=True)
    ...
    def start(self):
        ...
        rate = rospy.Rate(100)  # Increase frequency to 100 Hz
        # rate = rospy.Rate(50)
    ...
    ## Please refer back to the above code if time measurements are not needed
    
    def run_loop(self, ip):
    run_loop_start_time = time.time()  # Timer for the entire run_loop
    self.goal_event.wait()
    if rospy.is_shutdown() or ip is None:
        return None, None

    with self.reset_lock:
        goal_check_time_start = time.time()
        if not self.goal_event.is_set():
            return None, None
        self.logger.info(f"Time for goal_event check: {time.time() - goal_check_time_start:.4f} seconds")

        if ip is not None:
            step_time_start = time.time()
            result = self.rhctrl.step(ip)
            self.logger.info(f"Time for rhctrl.step: {time.time() - step_time_start:.4f} seconds")
            return result

    self.logger.err("Shouldn't get here: run_loop")
    self.logger.info(f"Total run_loop time: {time.time() - run_loop_start_time:.4f} seconds")
    return None, None
    ...
    def publish_traj(self, traj, rollout):
        ...
        ctrlmsg.drive.speed = min(ctrl[0], self.params.get_float("max_speed", default=2.0))  # Ensure higher speed is allowed
