    ...
    void map_cb(const nav_msgs::OccupancyGrid& msg)
{
    ...
    // Measure the time for resizing the map
    ros::Time resize_start = ros::Time::now();
    Mat resized;
    resizeOccupancyGrid(msg, resized);
    ROS_INFO("Resizing the map took: %f seconds", (ros::Time::now() - resize_start).toSec());

    // Measure the time for initializing the environment
    if (env_ == nullptr)
    {
        ros::Time env_init_start = ros::Time::now();
        initializeEnv(msg, resized);
        ROS_INFO("Environment initialization took: %f seconds", (ros::Time::now() - env_init_start).toSec());
    }

    // Measure the time for updating the costmap
    ros::Time costmap_update_start = ros::Time::now();
    for (int env_iy = 0; env_iy < resized.rows; ++env_iy)
    {
        for (int env_ix = 0; env_ix < resized.cols; ++env_ix)
        {
            uchar val = resized.data[env_iy * resized.cols + env_ix];
            if (val != 0)  // Only update obstacle cells
            {
                val = 255;
                env_->UpdateCost(env_ix, env_iy, val);
            }
    ...
    ROS_INFO("Costmap update took: %f seconds", (ros::Time::now() - costmap_update_start).toSec());

    // Total time for the function
    ROS_INFO("Total map_cb processing time: %f seconds", (ros::Time::now() - start_time).toSec());
}
    ...
    // Please refer back to above code snippet for errors
    ...
    planner_.reset(new ARAPlanner(env_.get(), forward_search_));
    planner_->set_search_mode(search_mode_);
    planner_->set_initialsolution_eps(3.0);  // Increased to speed up first solution
    replan_time_ = 0.3;  // Reduced planning time
    ROS_INFO("Planner initialized");
    runPlanner();
    }
